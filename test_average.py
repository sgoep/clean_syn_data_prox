...
for index in range(config.len_train, config.len_train+config.len_test):
    ...
    error_fbp_mse += mse(
        X.flatten(), Y.flatten())/config.len_test
    error_fbp_psnr += psnr(
        X.flatten(), Y.flatten(), data_range=Y.max() - Y.min())/config.len_test
    error_fbp_ssim += ssim(
        X.flatten(), Y.flatten(), data_range=Y.max() - Y.min())/config.len_test
    ...
    error_init_regul_mse += mse(
        X.flatten(), Y.flatten())/config.len_test
    error_init_regul_psnr += psnr(
        X.flatten(), Y.flatten(), data_range=Y.max() - Y.min())/config.len_test
    error_init_regul_ssim += ssim(
        X.flatten(), Y.flatten(), data_range=Y.max() - Y.min())/config.len_test
...
for model_name in [
    ...
    "fbp_nsn", f"{config.initial_regularization_method}_nsn",
    ...
    for index in range(config.len_train, config.len_train+config.len_test):
        ...
        model.load_state_dict(
            torch.load(
                'models/' + model_name, map_location=torch.device(device)
            )
        )
        model.eval()
        model.to(device)
        ...
        if index == config.len_train:
            ...
            errors[model_name]["MSE"] += mse(
                X.flatten(), out.flatten())/config.len_test,
            errors[model_name]["SSIM"] += ssim(
                X.flatten(),
                out.flatten(),
                data_range=out.max() - out.min())/config.len_test,
            errors[model_name]["PSNR"] += psnr(
                X.flatten(),
                out.flatten(),
                data_range=out.max() - out.min())/config.len_test
