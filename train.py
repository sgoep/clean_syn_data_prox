...
def train(model, TrainDataGenerator, TestDataGenerator, error, optimizer, device, NumEpochs, len_train, len_test):
...
def start_training(constraint: bool, initrecon: bool, null_space: bool, model_name: str, model_params: dict, training_params: dict, ell2_norm=None):
    
    model = UNet(n_channels=1, n_classes=1, constraint=constraint, null_space=null_space, norm2=ell2_norm).to(device)
    
    torch.manual_seed(42)
    model.apply(init_weights)
     
    model_optimizer = optim.Adam(model.parameters(), lr=training_params["learning_rate"])
    model_error     = nn.MSELoss()
                       
    model_train_set = DataLoader([i for i in range(training_params["len_train"])],
                                 constraint,
                                 initrecon,
                                 config.which)
    
    model_test_set  = DataLoader([i for i in range(training_params["len_train"], training_params["len_train"]+training_params["len_test"])],
                                 constraint,
                                 initrecon,
                                 config.which)
...
if __name__ == "__main__":
    ...
    #                initrecon = True,
    #                null_space = False,
    #                model_name = f"{config.which}_resnet", 
    #                model_params = model_params,
    #                training_params = training_params)
    ...
    #                initrecon = True,
    #                null_space = True,
    #                model_name = f"{config.which}_nsn",
    #                model_params = model_params,
    #                training_params = training_params)
    ...
    #                initrecon = True, 
    #                null_space = True,
    #                model_name = f"{config.which}_dpnsn",
    #                model_params = model_params,
    #                training_params = training_params,
    #                ell2_norm = ell2_norm)
