...
from radon_operator import (RadonOperator, RadonOperator_small, get_matrix, get_real_matrix,
                            ram_lak_filter)
...
def soft(c: List, a: float):
    ...
    Parameters:
    c (List): A list where the first element is a scalar and the subsequent elements are tuples of numerical values.
    a (float): The thresholding parameter.

    Returns:
    List: A new list with the same structure as `c`, where each element in the tuples has been soft-thresholded.
    """
    # cnew = [c[0]]
...
def my_multiply(c: List, a: float) -> List:
    ...
    Returns:
    List: A new list with the same structure as `c`, where each element in the tuples has been multiplied by `a`.
    """
    h = [c[0]*a]
    for k in c[1:]:
...
def my_subtract(c: List, a: List) -> List:
    ...
    Parameters:
    c (List): The first list, where the first element is a scalar and the subsequent elements are tuples of numerical values.
    a (List): The second list, with the same structure as `c`.

    Returns:
    List: A new list where each element is the result of subtracting corresponding elements of `a` from `c`.
    """
    h = [c[0] - a[0]]
...
def my_addition(c: List, a: List) -> List:
    ...
    Returns:
    List: A new list where each element is the result of adding corresponding elements of `c` and `a`.
    """
    h = [c[0] + a[0]]
    for k, j in zip(c[1:], a[1:]):
...
def fista(x0: np.ndarray, 
    ...
    """
    Perform the Fast Iterative Shrinkage-Thresholding Algorithm (FISTA) for solving linear inverse problems.
    ...
    for k in range(Niter):
        ...
        if ground_truth is not None:
            er[k] = np.sum(np.abs(Q(xout) - ground_truth)**2)/np.sum(np.abs(ground_truth)**2)
        if print_flag:# and ((k-1 % 100 == 0) or k==0):
            if (np.mod(k+1, 100) == 0) or (k==0):
                norm = np.linalg.norm(Q(xout))
                # print('Synthesis Iteration: ' + str(k+1) + '/' + str(Niter) + ', Error:' + str(er[k]))# + ', Norm: ' + {)
                print(f"Synthesis Iteration: {str(k+1)} / {str(Niter)}, Error: {str(er[k])}, Norm: {norm}")
    return xout
...
# Amat = torch.load('A_matrix.pt', map_location=torch.device('cpu')).to_dense().numpy()
# Amat = np.zeros([181*560, 512**2])
